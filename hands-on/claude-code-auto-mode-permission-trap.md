...
> 只想立刻把手头的活干完、不关心排查过程的话，直接看[《这条报错怎么解决：四步处理 + 变体速查》](/zh/articles/claude-code-auto-mode-temporarily-unavailable-fix)；那篇还回答了「auto 是什么模型」「bash denied by auto mode 是不是同一回事」「Shift+Tab 六种权限模式分别是什么」。
